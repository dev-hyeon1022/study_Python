from crud import save, find_all, find_one_id, update, delete


# insert_query = "insert into tbl_member(member_id, member_password, member_name)" \
#                "values(%s, %s, %s)"
#
# insert_param = ['lss5555', '5555', '이순신']


# save(insert_query, insert_param)

# 전체 조회
# select_query = "select id, member_id, member_password, member_name from tbl_member"
# print(find_all(select_query))

# 아이디가 'lsh123'인 회원의 이름을 '림수현'으로 수정
# update_query = 'update tbl_member set member_name = %s where member_id = %s'
# update_param = ['림수현', 'lsh123']
#
...
# select_query_one = 'select id, member_id, member_password, member_name from tbl_member where id = %s'
# select_query_id = ['2']
# print(find_one_id(select_query_one, select_query_id))

#단일 테이블 3개 제작후 CRUD 진행

# insert
# insert_query = 'insert into tbl_student(student_name, student_major, student_level)' \
#                'values(%s, %s, %s)'
# insert_param = ['임수현', '게임개발학과', '1']
# save(insert_query, insert_param)

#select
# select_query = 'select stundent'
